...
export function PublicBusinessPage({ branchSlug }: { branchSlug?: string }) {
  ...
  return (
    <main className="min-h-screen bg-[#F9FAFB] text-slate-950">
      <section className="bg-blue-600 px-4 py-8 text-white sm:px-6 lg:px-8">
        <div className="mx-auto max-w-6xl">
          ...
          <p className="mt-2 max-w-xl text-blue-100">Datang pas giliranmu. Cek antrean realtime dan ambil nomor dari HP.</p>
        </div>
      </section>

      <section className="mx-auto -mt-6 grid max-w-6xl gap-5 px-4 pb-12 sm:px-6 lg:grid-cols-[1fr_340px] lg:px-8">
        <div className="space-y-5">
          <Card className="rounded-[28px] border-0 bg-white shadow-xl shadow-slate-900/10">
        ...
        <Card className="h-fit rounded-[28px] border-slate-100 bg-white shadow-sm">
          <CardContent className="p-5">
            <div className="grid size-16 place-items-center rounded-3xl bg-blue-50 text-blue-700">
              <Scissors className="size-8" />
            </div>
            <h2 className="mt-5 text-2xl font-black">Pilih layanan</h2>
            <div className="mt-4 space-y-3">
              {data.services.map((service) => (
                <div key={service.id} className="flex items-center justify-between rounded-2xl border border-slate-100 p-4">
                  <div>
                    <p className="font-black">{service.name}</p>
                    <p className="text-sm font-semibold text-slate-500">{service.durationMinutes} menit</p>
                  </div>
                  <span className="text-sm font-black text-blue-700">Rp{service.price?.toLocaleString("id-ID")}</span>
                </div>
              ))}
            </div>
            <div className="mt-5 grid gap-3">
              <Button className="h-12 rounded-2xl bg-blue-600 font-black hover:bg-blue-700">Ambil Nomor</Button>
            ...
            <div className="mt-5 flex items-center gap-3 rounded-2xl bg-slate-50 p-4">
              ...
              <p className="text-sm font-bold text-slate-600">QR cabang siap dicetak dari dashboard.</p>
            </div>
          </CardContent>
        </Card>
      </section>
...
function Info({ icon: Icon, label, value }: { icon: typeof Clock3; label: string; value: string }) {
  return (
    <div className="flex items-center gap-3 rounded-[22px] bg-slate-50 p-4">
      <span className="grid size-11 place-items-center rounded-2xl bg-white text-blue-700 shadow-sm">
