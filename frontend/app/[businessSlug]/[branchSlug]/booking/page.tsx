    ...
    <main className="grid min-h-screen place-items-center bg-background px-4 py-10 text-foreground">
      <Card className="w-full max-w-xl rounded-[28px] shadow-xl shadow-slate-900/5 dark:shadow-black/20">
        <CardContent className="p-6">
          ...
          <p className="mt-2 text-muted-foreground">Pilih layanan, staff, tanggal, dan jam. Form ini siap dihubungkan ke endpoint booking.</p>
          <div className="mt-6 grid gap-3">
            {["Layanan", "Staff", "Tanggal", "Jam"].map((label) => (
              <div key={label} className="rounded-2xl border border-border p-4 font-bold text-muted-foreground">{label}</div>
            ))}
          </div>
          <Button className="mt-6 h-12 w-full rounded-2xl font-black">Konfirmasi Booking</Button>
        </CardContent>
      </Card>
    </main>
  );
